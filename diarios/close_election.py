...
def is_close(df: pd.DataFrame, only_two: bool = False) -> pd.Series:
    ...
    if not all(c in df.columns for c in cols):
        raise Exception('df needs cols:', cols)
    df_copy = copy(df)
    df_copy['group'] = _get_group(df_copy)
    df_copy = _rank_candidates(df_copy)
    df_copy = _center_rank(df_copy)
    df_copy['close'] = _is_close(df_copy, only_two)
    df_copy = _drop_duplicates(df_copy)
    if only_two:
        df_copy = _balance_close(df_copy, 'close', n=2)
    else:
        df_copy = _balance_close(df_copy, 'close')
...
def _get_group(df: pd.DataFrame) -> pd.Series:
    ...
    # office, year, round,
    # municipio_id, estado
    office_type = get_office_type(df.office)
    df['temp'] = np.where(office_type == 'pr', df.coalition, '')
    df['district'] = get_district(df)
    group = df.groupby(['year', 'district', 'office', 'round',
                        'temp']).ngroup()
    df.drop(columns=['temp', 'district'], inplace=True)
    return group
...
def _is_close(df: pd.DataFrame, only_two: bool = False) -> pd.Series:
    ...
    if only_two:
        close = ((close == 1) & df.crank.isin([-1, 1])) * 1
    return close
...
def _balance_close(
    ...
    df['emargin'] = (df.margin * df.electeddummy * df[var])
    df['nmargin'] = (df.margin * (1 - df.electeddummy) * df[var])
    emargin = (df.groupby('group')['emargin'].transform('min'))
    nmargin = (df.groupby('group')['nmargin'].transform('max'))
    df = df.drop(columns=['eclose', 'nclose', 'emargin', 'nmargin'])
    cond = (((out.eclose != out.nclose) | (emargin != -nmargin)) &
            (df[var] == 1))
    print(sum(cond), var, 'not balanced, setting to 0')
    if sum(cond) > 0:
        print('Example 1:')
    ...
    df.loc[cond, var] = 0
    if n:
        nclose = (df.groupby('group')[var].transform('sum'))
        notn = ((nclose != n) & (nclose != 0))
        print(sum(notn), var, 'not', n, 'setting to 0')
        if sum(notn) > 0:
            print('Example 1:')
...
def _drop_duplicates(df: pd.DataFrame) -> pd.DataFrame:
    ...
    cols = ['cpf', 'year', 'municipio_id', 'office', 'round']
    duplicated = df.duplicated(cols)
    print(sum(duplicated), "duplicates.", 'Dropping 1')
