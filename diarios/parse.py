...
class CaseParser:
    ...
    def parse(self, df):
        ...
        self.cleaners = {
            k: v
            for k, v in self.cleaners.items() if k in df.columns
        }
    ...
    def _add_cols(self, df):
        ...
        df['proc_id'] = clean.generate_id(df.number,
                                          suffix=self.id_suffix,
                                          suffix_length=self.suffix_length)
        df['mov_id'] = df.index
        df['mov_id'] = clean.generate_id(df.mov_id,
                                         suffix=self.id_suffix,
                                         suffix_length=self.suffix_length)
        return df
    ...
    def _split_parte(self, df):
        ...
        df = split_col(df,
                       'lastname',
                       split_on=self.split_parte_on,
                       group_id='lastname_group')
        df['name_group'] = np.where(df.name_group == '', df.lastname_group,
                                    df.name_group)
