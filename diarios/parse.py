...
class CaseParser:
    ...
    def __init__(self,
                 regexes=['(?P<number>[0-9.\-]{19,30})'],
                 regexes_before_split=None,
                 cleaners={'classe': clean.clean_classe},
                 parte='AUTOR:|RÉU:',
                 advogado=None,
                 split_parte_on=',|-|;',
                 split_text_on=None,
    ...
    def _split_text(self, df):
        if self.split_text_on:
            df = split_col(df, 'text',
                           split_on=self.split_text_on).reset_index()
        return df
    ...
    def _add_cols(self, df):
        ...
        df['number'] = self.clean_number(df.number)
        df['proc_id'] = clean.generate_id(df.number, suffix=self.id_suffix)
        df['mov_id'] = df.index
        df['mov_id'] = clean.generate_id(df.mov_id, suffix=self.id_suffix)
        return df

    def _get_parte(self, df):
        df_id = df.loc[:, self.parte_levels]
        if self.advogado:
            adv = df.text.str.extractall(self.advogado).reset_index('match',
                                                                    drop=True)
            adv['key'] = 'advogado'
        ...
        df = df.join(df_id)
        df = df.drop_duplicates(['parte', 'tipo_parte_id'] + self.parte_levels)
        return df.loc[:, ['parte', 'key', 'tipo_parte_id'] + self.parte_levels]

    def _split_parte(self, df):
        df = split_col(df, 'name', split_on=self.split_parte_on)
        df = split_col(df, 'lastname', split_on=self.split_parte_on)
        return df
    ...
    def _clean_parte_name(self, df):
        ...
        return np.where(df['name'] == '', df['lastname'], df['name'])
    ...
    def _get_proc(self, df):
        ...
        proc = (df.loc[:, cols].drop_duplicates('proc_id'))
...
def clean_diario_proc(proc, number_types=['CNJ']):
    proc['tribunal_id'] = clean.transform(proc['tribunal'], 'tribunal',
                                          'tribunal_id')
...
def parse_diario_extract(infile, nchar=None):
    ...
    split_regex = ('{}(?=[0-9]{{4}}/'
                   '[0-9]{{2}}/[0-9]{{2}}/)'.format(tribunal))
    df = (pd.Series(re.split(split_regex, text)).str.replace(
        ';', '').str.replace(r'^([0-9]{4}/[0-9]{2}/[0-9]{2})/', r'\1;',
                             n=3).str.replace(r'\.md', r';',
                                              n=1).str.replace(
                                                  r'(-|:)([0-9]+)(-|:)',
                                                  r'\2;',
                                                  n=1).str.split(';',
...
def get_keyword_regex(keyword, max_name_length=100, last_name_length=50):
    ...
    regex = ('(?s)(?P<key>{0})'
             '((?P<name>{1})|(?P<lastname>{2}))').format(
                 keyword, name, last_name)
    return regex


def inspect(proc, parte, mov, tp='parte', min_mov_length=100):
    mov = (mov.loc[mov.text.str.len() > min_mov_length].merge(
        proc.reset_index().loc[:, 'proc_id'],
        on='proc_id').merge(parte.loc[:, 'proc_id'],
                            on='proc_id').drop_duplicates('proc_id'))
    ex = mov.sample().iloc[0]
    proc_id = ex.proc_id
    print(ex['text'])
    prt = (parte.query('proc_id == {}'.format(proc_id)).loc[:,
                                                            ('parte', 'key')])
