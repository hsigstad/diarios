...
def query(database, sql, echo=True):
    conn = get_db_engine(database, echo=echo).connect()
...
def insert(database, table, files, outdir='build/insert', clean=None):
    ...
    for infile in files:
        print(infile)
        data = _get_data(infile, clean)
        chunksize = 1000
        nchunks = len(data) // chunksize + 1
        for chunk in range(0, nchunks):
            print('{} of {}'.format(chunk, nchunks))
            session.bulk_insert_mappings(
                table, data[chunk * chunksize:(chunk + 1) * chunksize])
    session.commit()
    engine.execute('SET FOREIGN_KEY_CHECKS=1')
    outfile = '{}/{}.txt'.format(outdir, table.__tablename__)
...
def _get_data(infile, clean=None):
...
def head(table, n=5):
    return pd.read_sql('SELECT * FROM {0} LIMIT {1}'.format(table, n),
                       session.bind)


def create_index(database, table, columns, name, index_type=''):
    '''
    Args:
    ...
    sql = ('CREATE {0} INDEX {1}\n'
           'ON {2} ({3})'.format(index_type, name, table, cols))
    conn.execute(sql)


def order_by(database, table, columns):
    conn = get_db_engine(database).connect()
    cols = ', '.join(columns)
    sql = ('ALTER TABLE {0} ORDER BY {1}'.format(table, cols))
    conn.execute(sql)
